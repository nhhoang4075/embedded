...
/* Queue carrying steering commands from defaultTask to the GUI task */
...
static void MX_GPIO_Init(void)
{
  ...
  /* PG2 (left) and PG3 (right): car control buttons, input with internal pull-up */
...
void StartDefaultTask(void *argument)
{
  ...
  for(;;)
  {
    /* Poll the two push buttons: PG2 = left, PG3 = right (pull-up, pressed = LOW).
       Both pressed at the same time -> 'B' (used on the menu screen to start the game). */
    uint8_t data = 0;
    GPIO_PinState left  = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_2);
    GPIO_PinState right = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_3);
    ...
    if (data != 0)
    {
      if (osMessageQueueGetCount(Queue1Handle) < 4)
      {
        osMessageQueuePut(Queue1Handle, &data, 0, 0);
      }
    }
    osDelay(5);
  }
